...
The dashboard reads `/api/check-status`. On Vercel, `vercel.json` runs that endpoint every two minutes. Status checks and history are stored in Supabase; the endpoint uses a short in-memory response cache so page loads do not re-ping every endpoint.
...
The service-role key is server-only and must not be exposed to browser code. Without these variables, local development uses an in-memory fallback for testing.
